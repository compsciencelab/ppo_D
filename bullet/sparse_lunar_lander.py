...
class LunarLanderSparse(gym.Env, EzPickle):
    metadata = {
    ...
    def step(self, action):
        ...
        #if self.step_counter == self.max_time_steps:
        #    return np.array(state, dtype=np.float32), self.cum_reward, True, {}
        #else:
        #     if done:
        #         return np.array(state, dtype=np.float32), self.cum_reward, done, {}
        #     else:
...
class LunarLanderContinuous(LunarLanderSparse):
    continuous = True
